...
const EditFlat = () => {
  ...
  const [formData, setFormData] = useState({
    apartmentName: '', bhkType: '', floors: '', totalFloors: '', area: '',
    location: { city: '', locality: '', landmark: '', coordinates: { lat: '', lng: '' } },
    rentPrice: '', deposit: '', negotiable: false, maintenance: false, maintenanceCost: '',
    preferredFor: '', furnishing: '', parking: '', description: '', bathrooms: '',
    amenities: [], images: [], availabilityDate: '', status: 'available',
  });
  ...
  useEffect(() => {
    ...
    const fetchFlat = async () => {
      try {
        ...
        const postedById = flat.postedBy._id || flat.postedBy;
        ...
        setFormData({
          ...
          location: {
            ...
            locality: flat.location?.locality || '',
            landmark: flat.location?.landmark || '',
            coordinates: flat.location?.coordinates || { lat: '', lng: '' },
          },
          rentPrice: flat.rentPrice || '',
          deposit: flat.deposit || '',
          negotiable: flat.negotiable || false,
          maintenance: flat.maintenance || false,
          maintenanceCost: flat.maintenanceCost || '',
          preferredFor: flat.preferredFor || '',
          ...
          images: flat.images || [],
          availabilityDate: flat.availabilityDate ? flat.availabilityDate.split('T')[0] : '',
          status: flat.status || 'available',
        });
        setMapPosition(flat.location?.coordinates?.lat ? {
          lat: flat.location.coordinates.lat,
          lng: flat.location.coordinates.lng,
        } : null);
      } catch (error) {
        console.error('Fetch Flat Error:', error);
        alert(`Failed to load flat details: ${error.message || 'Unknown error'}`);
        navigate('/dashboard');
      }
    };
    fetchFlat();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert(`Failed to update flat: ${error.message || 'Unknown error'}`);
    }
  };

  const handleCancel = () => {
    if (window.confirm('Are you sure you want to cancel editing this flat?')) {
      navigate('/dashboard');
    }
  };
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const uploadedUrls = await flatService.uploadPhotos(formDataUpload);
      setFormData(prev => ({
        ...prev,
        images: [...(prev.images || []), ...uploadedUrls.imageUrls],
      }));
    } catch (error) {
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="relative z-10 rounded-lg border border-gray-200 shadow-md">
                  <MapContainer
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    {formData.images.map((url, index) => (
                      <div key={index} className="relative rounded-lg overflow-hidden shadow-sm">
                        <img
                          src={`${url.startsWith('http') ? url : `${import.meta.env.VITE_BASE_URL}/Uploads/${url}`}`}
                          alt={`Photo ${index}`}
  ...
  if (!isAuthenticated) {
    return (
      <div className="text-center py-10">
        <p className="text-gray-600 mb-4">Please log in to edit a flat.</p>
        <button
