  ...
  useEffect(() => {
    const fetchServices = async () => {
      ...
      try {
        ...
        setServices(Array.isArray(data) ? data.filter((service) => service.serviceType === 'cleaning') : []);
      } catch (err) {
        setError(err.response?.data?.message || 'An error occurred while fetching cleaning services');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      {loading ? (
        <div className="text-center py-10">
          ...
          <p className="text-gray-600">Loading cleaning services...</p>
        </div>
      ) : error ? (
