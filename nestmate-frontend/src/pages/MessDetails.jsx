...
import { Coffee, MapPin, Star, Phone, Edit, Trash2, CheckCircle, X, ChevronLeft, ChevronRight, Users, Heart } from 'lucide-react';
...
const MessDetails = () => {
  ...
  const [currentImageIndex, setCurrentImageIndex] = useSlideshow(mess?.images, 3000);

  const isOwner = useMemo(() => {
    return isAuthenticated && user && mess && (mess.providerId._id === user.id || mess.providerId === user.id);
  }, [isAuthenticated, user, mess]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setLoading(true);
      try {
        const messData = await messService.getMessById(id);
        setMess(messData);
        const ratingsData = await ratingService.getRatings('mess', id);
        setRatings(Array.isArray(ratingsData) ? ratingsData : []);
        if (isOwner) {
          const contacted = await messService.getContactedUsers(id);
        ...
        alert(`Failed to load mess: ${error.message || 'Unknown error'}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleContact = () => {
    const phone = mess?.providerId?.phone;
    if (!phone) {
      alert("Owner's phone number is not available.");
      return;
    }
    const message = encodeURIComponent(`Hi, I'm interested in your mess "${mess.messName}" on NestMate. Can we discuss further?`);
    window.open(`https://wa.me/+91${phone}?text=${message}`, '_blank');
    contactService.createContact({
      contactedId: mess.providerId._id || mess.providerId,
      message: `Interested in your mess ${mess.messName}`,
      type:'mess',
  ...
  const handleConfirmContact = async (userId) => {
    try {
      await messService.confirmContactedUser(id, userId);
      const contacted = await messService.getContactedUsers(id);
      setContactedUsers(Array.isArray(contacted) ? contacted : []);
    } catch (error) {
  ...
  const handleRemoveContact = async (userId) => {
    if (
      window.confirm(
        `The person will not be able to rate your mess "${mess?.messName}". This cannot be undone.`
      )
    ) {
      try {
        await messService.removeContactedUser(id, userId);
        const contacted = await messService.getContactedUsers(id);
        setContactedUsers(Array.isArray(contacted) ? contacted : []);
      } catch (error) {
  ...
  const handleSubmitRating = async () => {
    try {
      ...
      setRating(0);
      setComment('');
      const updatedRatings = await ratingService.getRatings('mess', id);
  ...
  const handleEdit = () => navigate(`/edit-mess/${id}`);
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this mess?')) {
      try {
        await messService.deleteMess(id);
        alert('Mess deleted!');
        navigate('/dashboard');
      } catch (error) {
        alert(error.message);
  ...
  const navigateToPrevImage = (e) => {
    e.stopPropagation();
    if (mess?.images && mess.images.length > 1) {
      setCurrentImageIndex((prev) => (prev > 0 ? prev - 1 : mess.images.length - 1));
    }
  };
  ...
  const navigateToNextImage = (e) => {
    e.stopPropagation();
    if (mess?.images && mess.images.length > 1) {
      setCurrentImageIndex((prev) => (prev < mess.images.length - 1 ? prev + 1 : 0));
    }
  };
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
        <div className="bg-white rounded-lg shadow-lg p-10">
          <h2 className="text-2xl font-bold text-gray-800 mb-4">Mess Not Found</h2>
          <p className="text-gray-600 mb-6">The mess you're looking for doesn't exist or has been removed.</p>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="mb-6 flex items-center text-sm text-gray-500">
        ...
        <Link to="/search" className="hover:text-purple-600 transition-colors">Messes</Link>
        <span className="mx-2">›</span>
        <span className="text-gray-900">{mess.messName}</span>
      </div>
          ...
          <div>
            <div className="relative rounded-lg overflow-hidden bg-gray-100 shadow-md">
              {Array.isArray(mess.images) && mess.images.length > 0 ? (
                <>
                  <img
                    src={`${baseUrl}/uploads/${mess.images[currentImageIndex]}`}
                    alt={`Mess ${currentImageIndex}`}
                  ...
                  {mess.images.length > 1 && (
                    <>
                      ...
                      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-black bg-opacity-60 text-white px-3 py-1 rounded-full text-sm">
                        {currentImageIndex + 1} / {mess.images.length}
                      </div>
                    </>
                  )}
                </>
            ...
            {/* Thumbnail Gallery */}
            {Array.isArray(mess.images) && mess.images.length > 1 && (
          ...
          <div>
            ...
            <div className="flex items-center mb-4">
              ...
              <div className="flex items-center text-gray-600">
                ...
                <span className="text-sm">{mess.location.city}, {mess.location.locality}</span>
              </div>
            </div>

            <div className="mb-6">
              <div className="flex items-baseline mb-2">
                <h2 className="text-2xl font-bold text-gray-900">{formatUtils.formatCurrency(mess.ratePerMonth, 'INR')}</h2>
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-3 mb-6">
              <div className="flex items-center text-gray-700">
                <span className="w-28 text-sm text-gray-500">Timing:</span>
                <span className="font-medium">{mess.timings || 'Not specified'}</span>
              </div>
            </div>
      ...
      {mess.location.coordinates.lat && mess.location.coordinates.lng && (
        <section className="mt-10 bg-white rounded-lg shadow-md p-6 relative z-10">
          ...
          <div className="rounded-lg overflow-hidden border border-gray-200">
            <MapContainer
              center={[mess.location.coordinates.lat, mess.location.coordinates.lng]}
              zoom={13}
            ...
            >
              ...
              />
              <Marker position={[mess.location.coordinates.lat, mess.location.coordinates.lng]} />
            </MapContainer>
          </div>

          <div className="mt-4 text-gray-700">
            <p>
              <span className="font-medium">Address:</span> {mess.location.locality}, {mess.location.city}
              {mess.location.landmark && `, Near ${mess.location.landmark}`}
            </p>
          </div>
        </section>
      )}
      ...
      {isFullScreen && Array.isArray(mess.images) && mess.images.length > 0 && (
          ...
          {contactedUsers.length > 0 ? (
            ...
            <div className="text-center py-10">
              ...
              <p className="text-gray-600">
                When people contact you about this mess, they'll appear here.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="p-2 space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Tell others what you liked or disliked about this mess..."
