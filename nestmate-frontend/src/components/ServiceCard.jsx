import { Link } from 'react-router-dom';
import { Package, Coffee } from 'lucide-react';

const ServiceCard = ({ service }) => {
  const baseUrl = import.meta.env.VITE_BASE_URL; // Access VITE_BASE_URL from .env

  const getPricingText = (service) => {
    if (service.serviceType === 'cleaning') {
      return service.rate ? `₹${service.rate} / ${service.chargeType === 'perHour' ? 'hour' : 'BHK'}` : 'Pricing not available';
    } else if (service.serviceType === 'laundry') {
      return (service.chargePerShirt || service.chargePerCloth)
        ? `₹${service.chargePerShirt || 'N/A'} / shirt, ₹${service.chargePerCloth || 'N/A'} / cloth`
        : 'Pricing not available';
    }
    return 'Pricing not available';
  };

  const getServiceIcon = (serviceType) => {
    switch(serviceType) {
      case 'cleaning':
        return <Package size={24} className="text-blue-600" />;
      case 'laundry':
        return <Coffee size={24} className="text-purple-600" />;
      default:
        return <Package size={24} className="text-blue-600" />;
    }
  };

  // Construct full image URL if images exist
  const imageUrl = service.images && service.images[0] 
    ? `${baseUrl}/uploads/${service.images[0]}` 
    : null;

  return (
    <Link
      to={`/services/${service._id}`}
      className="block bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-all duration-300"
    >
      {imageUrl ? (
        <div className="relative">
          <img
            src={imageUrl}
            alt={service.name || service.serviceType}
            className="w-full h-48 object-cover"
            onError={(e) => { e.target.src = 'data:image/png;base64,'; }}
          />
          <div className="absolute top-0 right-0 m-2 px-2 py-1 bg-gradient-to-r from-blue-500 to-purple-600 rounded-md text-xs font-medium text-white">
            {service.serviceType === 'cleaning' ? 'Cleaning' : 'Laundry'}
          </div>
        </div>
      ) : (
        <div className="w-full h-48 bg-gray-100 flex flex-col items-center justify-center">
          {getServiceIcon(service.serviceType)}
          <span className="mt-2 text-gray-500 text-sm">No Image</span>
        </div>
      )}
      <div className="p-4">
        <div className="flex items-center">
          {getServiceIcon(service.serviceType)}
          <h3 className="ml-2 text-lg font-semibold bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-purple-600">
            {service.name || (service.serviceType === 'cleaning' ? 'Cleaning Service' : 'Laundry Service')}
          </h3>
        </div>
        <p className="mt-2 text-sm font-medium text-gray-800">{getPricingText(service)}</p>
        <div className="mt-2 p-3 bg-gray-50 rounded-lg">
          <p className="text-sm text-gray-600">
            {service.description 
              ? (service.description.length > 100 
                ? service.description.slice(0, 100) + '...' 
                : service.description) 
              : 'No description available'}
          </p>
        </div>
        <div className="mt-3 text-right">
          <span className="inline-flex items-center text-sm font-medium text-purple-600 hover:text-purple-800">
            View Details
            <svg className="ml-1 w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7"></path>
            </svg>
          </span>
        </div>
      </div>
    </Link>
  );
};

export default ServiceCard;